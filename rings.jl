...
module rings
    ...
    function strpth_record(nodcrt, lvlcrt, lvlprim, pths, strpth, strpthx, lvldist, lnks, nodlnkd)
        for lnkscrt in 1:lnks[nodcrt]
            ...
            elseif lvlprb == lvlcrt - 1
                ...
                pths, strpth, strpthx = strpth_record(nodprb, lvlprb, lvlprim, pths, strpth, strpthx, lvldist, lnks, nodlnkd)
            end
        end
    ...
    function prime_ring(iodd, lvlprim, lnks, lvldist, lvlref, ringstat, nodlnkd, querng, srtpth1, srtpth2, prime1, prime2, nodsrc, ngf, rings)
        rngs = size(querng)[1]
            ...
            if pth1 > 0
                for lvlmax in lvlprim:lvlprim+iodd
                    for lvlchk in 1:lvlmax-1
                        ...
                        goal_found = false
                        goal_found = pair_search(nodchk, nodmid, limit, goal_found, lvlref, lvldist, lnks, nodlnkd)
    ...
    function pair_search(nodcrt, nodgoal, limit, goal_found, lvlref, lvldist, lnks, nodlnkd)
        ...
        for lnkscrt in 1:lnks[nodcrt]
            ...
            for iref in 1:4 # could use more than four reference maps, if structures get big enough to require this
            ...
            goal_found = pair_search(nodprb, nodgoal, lmtx, goal_found, lvlref, lvldist, lnks, nodlnkd)
    ...
    function ring_statistics(numatoms, nodlnkd, refnodes, maxlvl=12, mxnodes=Int(1e4), mxpths=Int(100), progress=true, nods=nothing, parallel=false)
        ...
        lnks = [length(nodlnkd[m]) for m in 1:numatoms]
        ...
        for (ct, nodsrc) in enumerate(nods)
            if progress && Threads.threadid()==1
                if nodsrc % floor(length(nods)/20) == 0
                    print("$(floor(ct/length(nods) * 100))%..")
            ...
            lvlref = [dijkstra_nonwgt(i, maxlvl*2, lnks, nodlnkd) for i in refnodes] # need to set these sensibly automatically
            ...
            for prime in primes
                ...
                if size(findall([lvldist[n] == lvldist[prime]-1 for n in nodlnkd[prime]]))[1] > 1
                    ...
                    pths, strpth, strpthx = strpth_record(prime, lvldist[prime], lvldist[prime], pths, strpth, strpthx, lvldist, lnks, nodlnkd)
                    ...
                    ngf, rings, ringstat = prime_ring(0, lvldist[prime], lnks, lvldist, lvlref, ringstat, nodlnkd, querng, strpth, strpth, prime, prime, nodsrc, ngf, rings)
                end
                ...
                if size(equal_lvls)[1] > 0
                    ...
                    pths, strpth, strpthx = strpth_record(prime1, lvldist[prime1], lvldist[prime1], pths, strpth, strpthx, lvldist, lnks, nodlnkd)
                    for prime2 in nodlnkd[prime1][equal_lvls]
                        ...
                        pths2, strpth2, strpthx2 = strpth_record(prime2, lvldist[prime2], lvldist[prime2], pths2, strpth2, strpthx2, lvldist, lnks, nodlnkd)
                        ...
                        # check if is prime ring
                        ngf, rings, ringstat = prime_ring(1, lvldist[prime], lnks, lvldist, lvlref, ringstat, nodlnkd, querng, strpth, strpth2, prime1, prime2, nodsrc, ngf, rings)
                    end
    ...
    function ring_statistics_parallel(numatoms, nodlnkd, refnodes, maxlvl=12, mxnodes=Int(1e4), mxpths=Int(100), progress=true)
        ...
        tasks = map(chunks) do chunk
            Threads.@spawn ring_statistics(numatoms, nodlnkd, refnodes, maxlvl, mxnodes, mxpths, progress, chunk, true)
        end
        results = fetch.(tasks)
